...
function is_date_before_today( $date = false ) {
...
function days_ago( $date = false ) {

  if ( $date ) {
    $today = time();
    $difference = $date - $today;
    if ( $difference > 0 ) { $difference = 0; }
    return abs( floor( $difference/60/60/24 ) );
  }
...
function get_increment_ago( $date = false ) {
  $date = ( strstr( $date, ":" ) ) ? strtotime($date) : $date;
  $date = time_ago( $date );
  if( $date < 60 ) {
    return $date . " seconds ago";
  } else if ( $date < 3600 && $date > 60 ) {
    return abs(floor($date/60)) . " minutes ago";
  } else if ( $date < 86400 && $date > 3600 ) {
    return abs(floor($date/60/60)) . " hours ago";
  } else if ( $date < 604800 && $date > 86400 ) {
    return abs(floor($date/60/60/24)) . " days ago";
  } else if ( $date < 2.63e+6 && $date > 60400 ) {
    return abs(floor($date/60/60/24/7)) . " weeks ago";
  } else if ( $date < 3.156e+7 && $date > 2.63e+6 ) {
    return abs(floor($date/60/60/24/365)) . " years ago";
  }
}
...
function register_post( $args ) {
  if ( $args["unique"] == false  or !get_post_by_title( $args['post_title'] ) ) {
...
function get_post_id_from_slug( $post_name ) {
